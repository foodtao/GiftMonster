...
/* Hot Sale */
$('.mask').css('opacity', .6).eq(0).css('opacity', 0);
var HotSale = {
	_counter : 1,
	t: undefined,
	timer: function() {
		this._counter = this._counter < 8 ? this._counter : this._counter - 8;
		$('.mask').css('opacity', .6).eq(this._counter).animate({
			opacity: 0
		},1000);
		$('.detail-product-list').animate({
			marginLeft: 0 - 470 * this._counter
		},1000);
		this._counter++;
	},
	startTimer: function() {
		this.t = setInterval("HotSale.timer()", 5000); 
	},
	stopTimer: function() {
...
HotSale.startTimer();
$('.mask').click(function() {
	HotSale._counter = $('.mask').index(this);
	HotSale.stopTimer();
	HotSale.timer();
	HotSale.startTimer();
});
